...
final class AlertEngine {
    ...
    private func evaluate() async {
        ...
        // P1-A: Read both caps and current counts once before the loop.
        let alreadyFiredToday  = db.alertsFiredToday()
        let alreadyFiredWeek   = db.alertsFiredThisWeek()
        let remainingToday     = max(0, Constants.maxAlertsPerDay  - alreadyFiredToday)
        let remainingWeek      = max(0, Constants.maxAlertsPerWeek - alreadyFiredWeek)
        let globalRemaining    = min(remainingToday, remainingWeek)
        guard globalRemaining > 0 else { return }
        ...
        for item in userItems {
            // P1-E: Suppress restock alerts for seasonal items.
            ...
            // P1-F: Use store-scoped average for alert threshold; fall back to all-stores.
            let rollingAvg = storeID > 0
                ? db.rollingAverage90(for: item.itemID, storeID: storeID)
        ...
        // P1-A: gate on BOTH daily and weekly remaining counts.
        let toFire = Array(sorted.prefix(globalRemaining))
